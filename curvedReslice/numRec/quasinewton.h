template <class T> void dfpmin(VecDoub_IO &p, const Doub gtol, Int &iter, Doub &fret, T &funcd) {
  const Int ITMAX = 200;
  const Doub EPS = numeric_limits<Doub>::epsilon();
  const Doub TOLX = 4 * EPS, STPMX = 1000.0;
  Bool check;
...
template <class T> struct Funcd {
  Doub EPS;
  T &func;
  Doub f;
  Funcd(T &funcc) : EPS(1.0e-8), func(funcc) {}
  Doub operator()(VecDoub_I &x) { return f = func(x); }
