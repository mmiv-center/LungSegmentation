...
public:
  ...
  Vec operator+(const Vec &src) const {
    Vec tmp(*this);
    return (tmp += src);
  }
  Vec &operator-=(const Vec &src) {
  ...
  Vec &operator*=(const double src) {
    x *= src;
    y *= src;
    z *= src;
    return *this;
  }
  Vec operator*(const double src) const {
    Vec tmp(*this);
    return (tmp *= src);
  }
  Vec &operator/=(const double src) {
...
inline Vec operator*(const double src, const Vec &v) {
  Vec tmp(v);
  return (tmp *= src);
}
...
public:
  ...
  // Operators
  double operator()(unsigned column, unsigned row) { return data[column + (row * 4)]; }
};
...
Mtx rotateX(double ang) {
  double s = (double)sin(Deg2Rad(ang));
  double c = (double)cos(Deg2Rad(ang));
...
Mtx rotate(double pitch, double head, double roll) {
  double sp = (double)sin(Deg2Rad(pitch));
  double cp = (double)cos(Deg2Rad(pitch));
  double sh = (double)sin(Deg2Rad(head));
  double ch = (double)cos(Deg2Rad(head));
  double sr = (double)sin(Deg2Rad(roll));
  double cr = (double)cos(Deg2Rad(roll));
...
inline Vec operator*(const Vec &v, const Mtx &m) {
  return Vec(m.data[0] * v.x + m.data[1] * v.y + m.data[2] * v.z + m.data[3], m.data[4] * v.x + m.data[5] * v.y + m.data[6] * v.z + m.data[7],
...
class Plane {
public:
  enum PLANE_EVAL { EVAL_COINCIDENT, EVAL_IN_BACK_OF, EVAL_IN_FRONT_OF, EVAL_SPANNING };
  ...
  Plane(const Vec &normal, const Vec &pt) : normal(normal), d(dot(-normal, pt)) {}

  // Copy constructor
  Plane(const Plane &a) : normal(a.normal), d(a.d) {}
